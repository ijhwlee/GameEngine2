...
public class PlayerController : NetworkBehaviour
{
  ...
  public NetworkVariable<UnityEngine.Color> NetColor = new NetworkVariable<UnityEngine.Color>();
  ...
  void DoMove(Vector2 dir, long ticks)
  {
    ...
    //float deltaTime = Time.deltaTime + (float)deltaTicks/TimeSpan.TicksPerSecond;
    float deltaTime = (deltaTicks > 0 ? 0 : Time.deltaTime) + (float)deltaTicks / TimeSpan.TicksPerSecond;
    //Debug.Log("[PalyerController:CheckMove]moveSpeed : " + moveSpeed.ToString()+ ", deltaTime : "+ deltaTime+", ticks : "+ticks+", deltaTicks : "+deltaTicks+", IsServer : "+ NetworkManager.Singleton.IsServer);
    Vector3 move = new Vector3(dir.x, 0, dir.y) * moveSpeed * Time.deltaTime;
    transform.Translate(move);
    //MoveDirection.Value = dir; // we need to propagate only position, not direction since client do not move actually
    Position.Value = transform.position;
  }

  void DoJump(float jumpPower)
  {
    //Debug.Log("[PalyerController:DoJump]NetworkManager.Singleton.IsServer : " + NetworkManager.Singleton.IsServer + ", IsOwner : " + IsOwner);
